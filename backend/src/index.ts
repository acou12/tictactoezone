...
app.post('/login', (req, res): any => {
  if (!req.body.username || !req.body.password) return res.status(400).send("WRONG.")
  let theUser = users.filter(it => it.username === req.body.username && it.password === req.body.password)[0]
  if (!theUser) {
